...
class _IndicatorPainter extends CustomPainter {
  ...
  final CjTabBarIndicatorSize indicatorSize;
  ...
  Rect indicatorRect(Size CjtabBarSize, int tabIndex) {
    assert(_currentTabOffsets != null);
    ...
    if (indicatorSize == CjTabBarIndicatorSize.label) {
    ...
    return Rect.fromLTWH(tabLeft, 0.0, tabRight - tabLeft, CjtabBarSize.height);
  }
...
// This class, and CjTabBarScrollController, only exist to handle the case
// where a scrollable CjTabBar has a non-zero initialIndex. In that case we can
// only compute the scroll position's initial scroll offset (the "correct"
// pixels value) after the CjTabBar viewport width and scroll limits are known.
class _CjTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _CjTabBarScrollPosition({
  ...
  bool applyContentDimensions(double minScrollExtent, double maxScrollExtent) {
    ...
    if (_initialViewportDimensionWasZero != true) {
      ...
      correctPixels(CjtabBar._initialScrollOffset(viewportDimension, minScrollExtent, maxScrollExtent));
      result = false;
    }
...
// This class, and CjTabBarScrollPosition, only exist to handle the case
// where a scrollable CjTabBar has a non-zero initialIndex.
class _CjTabBarScrollController extends ScrollController {
  _CjTabBarScrollController(this.CjtabBar);

  final _CjTabBarState CjtabBar;

  @override
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition oldPosition) {
    return _CjTabBarScrollPosition(
      physics: physics,
      context: context,
      oldPosition: oldPosition,
      CjtabBar: CjtabBar,
...
/// See also:
///
///  * [CjTabBarView], which displays page views that correspond to each tab.
class CjTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// and the entire [CjTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// is [CjTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [CjTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  ...
  /// tab's overall bounds if [indicatorSize] is [CjTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [CjTabBarIndicatorSize.label].
  ///
  ...
  /// An optional callback that's called when the [CjTabBar] is tapped.
  ///
  /// The callback is applied to the index of the tab where the tap occurred.
  ///
  /// This callback has no effect on the default handling of taps. It's for
  /// applications that want to do a little extra work when a tab is tapped,
  /// even if the tap doesn't change the TabController's index. CjTabBar [onTap]
  ...
  @override
  _CjTabBarState createState() => _CjTabBarState();
}

class _CjTabBarState extends State<CjTabBar> {
  ...
  // If the CjTabBar is rebuilt with a new tab controller, the caller should
  ...
  @override
  void didUpdateWidget(CjTabBar oldWidget) {
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller.indexIsChanging && widget.isScrollable) {
      // Sync the CjTabBar's scroll position with the CjTabBarView's PageView.
      _currentIndex = _controller.index;
  ...
  Widget build(BuildContext context) {
      ...
      if (_controller.length != widget.tabs.length) {
        throw FlutterError(
            'Controller\'s length property (${_controller.length}) does not match the \n'
                'number of tabs (${widget.tabs.length}) present in CjTabBar\'s tabs property.'
        );
      }
      return true;
    }());
    ...
    if (_controller != null) {
      ...
      if (_controller.indexIsChanging) {
        ...
        // The user is dragging the CjTabBarView's PageView left or right.
    ...
    if (widget.isScrollable) {
      _scrollController ??= _CjTabBarScrollController(this);
      CjtabBar = SingleChildScrollView(
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [CjTabBar].
///
/// If a [TabController] is not provided, then there must be a [DefaultTabController]
/// ancestor.
///
/// The tab controller's [TabController.length] must equal the length of the
/// [children] list and the length of the [CjTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class CjTabBarView extends StatefulWidget {
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [CjTabBar.tabs]
  ...
  @override
  _CjTabBarViewState createState() => _CjTabBarViewState();
}

final PageScrollPhysics _kCjTabBarViewPhysics = const PageScrollPhysics().applyTo(const ClampingScrollPhysics());

class _CjTabBarViewState extends State<CjTabBarView> {
  TabController _controller;
  ...
  // If the CjTabBarView is rebuilt with a new tab controller, the caller should
  // dispose the old one. In that case the old controller's animation will be
  // null and should not be accessed.
  bool get _controllerIsValid => _controller?.animation != null;
  ...
  @override
  void didUpdateWidget(CjTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.controller != oldWidget.controller)
      _updateTabController();
  ...
  Widget build(BuildContext context) {
    assert(() {
      if (_controller.length != widget.children.length) {
        throw FlutterError(
            'Controller\'s length property (${_controller.length}) does not match the \n'
                'number of tabs (${widget.children.length}) present in CjTabBar\'s tabs property.'
        );
      }
      return true;
    }());
    return NotificationListener<ScrollNotification>(
      ...
      child: PageView(
        ...
        physics: widget.physics == null ? _kCjTabBarViewPhysics : _kCjTabBarViewPhysics.applyTo(widget.physics),
...
class TabPageSelector extends StatelessWidget {
  ...
  Widget _buildTabIndicator(
    ...
    if (tabController.indexIsChanging) {
      ...
      // The selection's offset reflects how far the CjTabBarView has / been dragged
...
class CjTab extends StatefulWidget {
  ...
  @override
  _CjTabState createState() => _CjTabState();
}

class _CjTabState extends State<CjTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: CjAdapt.screenW()/widget.rowNum,
      child: Center(
        child: label,
        widthFactor: 1.0,
      ),
